...
int selftest_run_parser(void)
{
    ...
    if (!parse_set_command("set cpu 45\n", fan, sizeof(fan), &percent) ||
        ...
        fprintf(stderr, "self-test failed: parse set cpu 45\n");
        failures++;
    }

    if (!parse_set_command(" set all 100 ", fan, sizeof(fan), &percent) ||
        strcmp(fan, "all") != 0 || percent != 100) {
        fprintf(stderr, "self-test failed: parse set all 100\n");
        failures++;
    }
    ...
    if (parse_set_command("set gpu 0", fan, sizeof(fan), &percent) ||
        ...
        fprintf(stderr, "self-test failed: reject malformed set command\n");
        failures++;
    }
    ...
    if (!parse_preset_command("preset balanced\n", preset_name,
        ...
        fprintf(stderr, "self-test failed: parse preset balanced\n");
        failures++;
    }

    if (parse_preset_command("preset balanced extra", preset_name,
                             sizeof(preset_name))) {
        fprintf(stderr, "self-test failed: reject malformed preset command\n");
        failures++;
    }
    ...
    if (!parse_power_source_auto_command("power-source auto on\n", action,
                                         sizeof(action)) ||
        strcmp(action, "on") != 0) {
        fprintf(stderr, "self-test failed: parse power-source auto on\n");
        failures++;
    }

    if (parse_power_source_auto_command("power-source auto on extra", action,
                                        sizeof(action))) {
        fprintf(stderr, "self-test failed: reject malformed power-source auto command\n");
        failures++;
    }

    if (!parse_gpu_temp_command("gpu-temp live\n", action, sizeof(action)) ||
        strcmp(action, "live") != 0) {
        fprintf(stderr, "self-test failed: parse gpu-temp live\n");
        failures++;
    }

    if (!parse_gpu_temp_command("gpu-temp status", action, sizeof(action)) ||
        strcmp(action, "status") != 0) {
        fprintf(stderr, "self-test failed: parse gpu-temp status\n");
        failures++;
    }

    if (parse_gpu_temp_command("gpu-temp live extra", action,
                               sizeof(action))) {
        fprintf(stderr, "self-test failed: reject malformed gpu-temp command\n");
        failures++;
    }
