                                     ...
                                     bool *coolboost_enabled)
{
    ...
    if (selftest_execute_command("set cpu 44\n", ec, cfg, states, auto_mode,
        ...
        *auto_mode || strcmp(preset, "manual") != 0 ||
        ...
        fprintf(stderr, "self-test failed: set cpu command path\n");
        failures++;
    }

    if (selftest_execute_command("set all 48\n", ec, cfg, states, auto_mode,
                                 preset, preset_len, coolboost_enabled,
                                 true, reply, sizeof(reply)) < 0 ||
        ...
        fprintf(stderr, "self-test failed: set all command path\n");
        failures++;
    }

    if (selftest_execute_command("preset max\n", ec, cfg, states, auto_mode,
                                 preset, preset_len, coolboost_enabled,
                                 true, reply, sizeof(reply)) < 0 ||
        ...
        *auto_mode || strcmp(preset, "max") != 0 ||
        ...
        fprintf(stderr, "self-test failed: preset command path\n");
        failures++;
    }

    if (selftest_execute_command("auto\n", ec, cfg, states, auto_mode,
                                 preset, preset_len, coolboost_enabled,
                                 true, reply, sizeof(reply)) < 0 ||
        !strstr(reply, "mode=auto preset=auto") ||
        !*auto_mode || strcmp(preset, "auto") != 0 ||
        states[0].percent != 50 || states[1].percent != 50) {
        fprintf(stderr, "self-test failed: auto command path\n");
        failures++;
    }

    if (selftest_execute_command("resume\n", ec, cfg, states, auto_mode,
                                 preset, preset_len, coolboost_enabled,
                                 true, reply, sizeof(reply)) < 0 ||
        !strstr(reply, "resume=ok mode=auto preset=auto")) {
        fprintf(stderr, "self-test failed: resume command path\n");
        failures++;
    }
