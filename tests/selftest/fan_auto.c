                               ...
                               fan_state states[ANS_MAX_FANS])
{
    ...
    if (states[0].percent != 42 || states[1].percent != 37 ||
        ...
        fprintf(stderr, "self-test failed: auto ramped shared max control temperature\n");
        failures++;
    }
    ...
    if (states[0].percent != 55 || states[1].percent != 55) {
        fprintf(stderr, "self-test failed: auto ramp reaches curve target\n");
        failures++;
    }
    ...
    if (auto_ramped_percent(cfg, &states[0], 100, "") != 62 ||
        ...
        fprintf(stderr, "self-test failed: auto ramp limit without temperature bypass\n");
        failures++;
    }

    if (auto_ramped_percent(cfg, &states[0], 100, "critical-temperature") != 100) {
        fprintf(stderr, "self-test failed: auto ramp forced bypass\n");
        failures++;
    }
    ...
    if (auto_ramped_percent(cfg, &states[0], 100, "") != 100) {
        fprintf(stderr, "self-test failed: configured auto ramp temperature bypass\n");
        failures++;
    }
