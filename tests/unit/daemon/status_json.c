...
/**
 * Verify daemon status JSON formatting.
 *
...
int unit_run_status_json(struct ec_device* ec, const struct ans_config* cfg, fan_state states[ANS_MAX_FANS])
{
    ...
    snprintf(local_cfg.fans[0].name, sizeof(local_cfg.fans[0].name), "CPU \"Fan\"");
    snprintf(local_cfg.fans[1].name, sizeof(local_cfg.fans[1].name), "GPU \\ Fan");

    snprintf(names.cpu, sizeof(names.cpu), "Intel\nCore");
    snprintf(names.gpu, sizeof(names.gpu), "NVIDIA\tGPU");

    snprintf(local_states[0].safety_reason, sizeof(local_states[0].safety_reason), "critical\treason");

    local_states[0].safety_active = true;

    ec_write_byte(ec, local_cfg.fan_modes.cpu_reg, local_cfg.fan_modes.cpu_auto_value);
    ec_write_byte(ec, local_cfg.fan_modes.gpu_reg, local_cfg.fan_modes.gpu_auto_value);
    ec_write_byte(ec, local_cfg.platform_profiles.reg, local_cfg.platform_profiles.profiles[1].value);
    ec_write_byte(ec, local_cfg.keyboard_backlight.reg, 2);

    if (!format_status_json(json, sizeof(json), &local_cfg, ec, local_states,false, "manual", false, &names, &runtime, 42))
    {
    ...
    if (!strstr(json, "\"name\": \"CPU \\\"Fan\\\"\"") || !strstr(json, "\"name\": \"GPU \\\\ Fan\""))
    {
    ...
    if (!strstr(json, "\"component_name\": \"Intel\\nCore\"") || !strstr(json, "\"component_name\": \"NVIDIA\\tGPU\""))
    {
