...
/**
 * Verify automatic fan ramp behavior.
 *
...
int unit_run_fan_auto_ramp(struct ec_device* ec, struct ans_config* cfg, fan_state states[ANS_MAX_FANS])
{
    ...
    if (states[0].percent != 42 || states[1].percent != 37 || states[1].temp_c != 45 || states[1].control_temp_c != 65)
    {
    ...
    if (auto_ramped_percent(cfg, &states[0], 100, "") != 62 || auto_ramped_percent(cfg, &states[0], 80, "") != 62)
    {
