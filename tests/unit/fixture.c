...
/**
 * Initialize core test model fields.
 *
...
/**
 * Initialize test fan-mode configuration.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
...
/**
 * Initialize test platform profiles.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
...
static void init_platform_profiles(struct platform_profile_config* profiles)
{
    ...
    snprintf(profiles->default_profile, sizeof(profiles->default_profile), "balanced");

    profiles->profile_len = 3;

    snprintf(profiles->profiles[0].id, sizeof(profiles->profiles[0].id), "quiet");

    profiles->profiles[0].value = 0x00;

    snprintf(profiles->profiles[1].id, sizeof(profiles->profiles[1].id), "balanced");

    profiles->profiles[1].value = 0x01;

    snprintf(profiles->profiles[2].id, sizeof(profiles->profiles[2].id), "performance");

    profiles->profiles[2].value = 0x04;
}

/**
 * Initialize test power-source profiles.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
...
static void init_power_source_profiles(struct power_source_profile_config* profiles)
{
    ...
    snprintf(profiles->battery_profile, sizeof(profiles->battery_profile), "quiet");
}

/**
 * Initialize test keyboard backlight support.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
...
/**
 * Initialize test CPU fan configuration.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
...
/**
 * Initialize test GPU fan configuration.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
...
/**
 * Reset fan states for each test.
 *
 * The unit suite keeps this behavior pinned without requiring Acer hardware,
 * so future refactors can change structure without changing the public
 * command contract.
