...
/**
 * Read configured firmware fan-mode values.
 *
 ...
 * pretending to work.
 */
static bool fan_mode_values(const struct fan_mode_config* fan_modes, const char* mode, int* cpu_value, int* gpu_value)
{
...
/**
 * Compute mode value name.
 *
 * Platform controls mirror Acer Sense features that are actually reachable on
 * Linux. Unsupported firmware surfaces should report clearly instead of
 * pretending to work.
 */
const char* fan_mode_value_name(const struct fan_mode_config* fan_modes, const bool cpu, const int value)
{
    if (cpu)
    {
...
{
    int cpu_value, gpu_value;

    if (!cfg->fan_modes.available || !fan_mode_values(&cfg->fan_modes, mode, &cpu_value, &gpu_value))
        return false;

    return ec_write_byte(ec, cfg->fan_modes.cpu_reg, cpu_value) == 0 && ec_write_byte(ec, cfg->fan_modes.gpu_reg, gpu_value) == 0;
}
 ...
 * pretending to work.
 */
bool apply_coolboost(struct ec_device* ec, const struct ans_config* cfg, fan_state states[ANS_MAX_FANS], const bool enabled)
{
    if (!cfg->fan_modes.available)
        return false;
