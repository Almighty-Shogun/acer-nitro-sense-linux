...
bool handle_gpu_temp_command(const int client, const char *cmd)
{
    ...
    if (!parse_gpu_temp_command(cmd, action, sizeof(action))) {
        dprintf(client, "error usage: gpu-temp status|auto|live\n");
        return true;
    }
    ...
    if (changed <= 0) {
        dprintf(client, "error gpu-temp policy write failed policy=%s\n", policy);
        return true;
    }

    if (!daemon_quiet_logs)
        fprintf(stderr, "gpu_temp_policy policy=%s changed=%d\n", policy, changed);
    dprintf(client, "gpu_temp=available policy=%s live=%s changed=%d\n",
