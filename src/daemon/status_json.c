...
{
    text_buffer out;
    int fan_cpu_mode, fan_gpu_mode, platform_profile = -1;
    ...
    json_append_string(&out, fallback_text(cfg->power_source_profiles.ac_profile, "unavailable"));
    text_buffer_append(&out, ",\n  \"power_source_battery_profile\": ");
    json_append_string(&out, fallback_text(cfg->power_source_profiles.battery_profile, "unavailable"));
    ...
    json_append_string(&out, fallback_text(keyboard_backlight.name, "unavailable"));
    text_buffer_append(
        &out,
        ",\n"
    ...
    text_buffer_append(
        ...
        cfg->keyboard_backlight.timeout_supported ? cfg->keyboard_backlight.timeout_seconds : 0
    );

    json_append_string(&out, cfg->keyboard_backlight.timeout_supported ? "input-activity" : "unsupported");
