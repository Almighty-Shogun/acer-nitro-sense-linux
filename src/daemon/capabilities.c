...
/**
 * Build the supported platform profile list.
 *
 ...
 * clients.
 */
static void platform_profile_list(const struct ans_config* cfg, char* out, const size_t out_len)
{
    size_t off = 0;
    ...
    {
        const int written = snprintf(out + off, out_len - off, "%s%s", i == 0 ? "" : ",", cfg->platform_profiles.profiles[i].id);

        if (written < 0 || (size_t)written >= out_len - off) break;
...
/**
 * Describe the active GPU temperature source.
 *
 * Daemon helpers own process lifetime and runtime coordination. They keep
 * privileged EC access behind one service instead of spreading it into
 * clients.
 */
static void gpu_temperature_source(const struct ans_config* cfg, char* out, const size_t out_len)
{
...
/**
 * Describe keyboard backlight control support.
 *
 * Daemon helpers own process lifetime and runtime coordination. They keep
 * privileged EC access behind one service instead of spreading it into
 * clients.
 */
static const char* keyboard_backlight_control(const struct ans_config* cfg, const struct keyboard_backlight_status* status)
{
 ...
 * clients.
 */
void reply_capabilities(const int client, const struct ans_config* cfg, const daemon_runtime_state* runtime)
{
    char profiles[256], gpu_power_control[16], gpu_temp_source[32];

    struct keyboard_backlight_status keyboard_backlight;

    const bool gpu_power_control_available =
        sensor_read_group_power_control("gpu", gpu_power_control, sizeof(gpu_power_control)) == 0;
    ...
    control_reply(
        ...
        gpu_power_control_available || strcmp(gpu_temp_source, "hwmon") != 0 ? "available" : "cached-only",
        gpu_temp_source,
    ...
    control_reply(
        ...
        availability_text(cfg->keyboard_backlight.available || keyboard_backlight.available),
        ...
        cfg->keyboard_backlight.available ? "ok" : keyboard_backlight_reason(&keyboard_backlight)
    );
}
