                  ...
                  const char *preset, const bool coolboost_enabled)
{
    ...
    dprintf(client, "model=%s mode=%s auto=%d preset=%s coolboost=%s power_source=%s\n", cfg->model,
    ...
    for (int i = 0; i < cfg->fan_len; i++) {
        ...
        dprintf(client, "%s rpm=%d temp=%d control=%s active_percent=%s requested=%d effective=%d percent=%d write_value=%d safety=%s%s%s critical_samples=%d ec_read_failures=%d ec_write_failures=%d\n",
                       ...
                       const bool auto_mode, const char *preset)
{
    ...
    for (int i = 0; i < cfg->fan_len; i++) {
        ...
        dprintf(client, "%s requested=%d effective=%d percent=%d\n",
...
void reply_presets(const int client, const struct ans_config *cfg)
{
    for (int i = 0; i < cfg->preset_len; i++)
        dprintf(client, "preset=%s cpu=%d gpu=%d\n",
...
void reply_ec_read(const int client, struct ec_device *ec, const int reg)
{
    ...
    if (value < 0) {
        dprintf(client, "error ec read failed register=0x%02x\n", reg);

        return;
    }

    dprintf(client, "ec[0x%02x]=0x%02x (%d)\n", reg, value, value);
}
