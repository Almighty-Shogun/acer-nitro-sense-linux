...
static void maybe_apply_power_source_profile(struct ec_device *ec,
                                             ...
                                             const daemon_runtime_state *runtime)
{
    ...
    if (source == POWER_SOURCE_UNKNOWN || source == *last_source)
        return;

    *last_source = source;

    if (!runtime || !runtime->power_source_auto_apply)
        return;

    profile = power_source_profile_for(cfg, source);
    if (!profile)
        return;
...
static void write_runtime_status(struct ec_device *ec, const struct ans_config *cfg,
                                 ...
                                 const daemon_runtime_state *runtime)
{
    write_status(cfg, ec, states, auto_mode, preset, coolboost_enabled, names,
                 runtime);
    write_temperature_cache(cfg, states);
}

void run_daemon_loop(const int sock_fd, struct ec_device *ec, const struct ans_config *cfg,
    ...
    while (daemon_running) {
        ...
        if (next_update_ms <= now_ms) {
            ...
            if (input_monitor.fd_len > 0)
                keyboard_backlight_timeout_tick(ec, cfg, runtime, now_ms,
                                                input_monitor.last_activity_ms);
            update_fan_states(ec, cfg, states, *auto_mode, preset);
            write_runtime_status(ec, cfg, states, *auto_mode, preset,
                                 *coolboost_enabled, names, runtime);
        ...
        if (ready > 0) {
            ...
            if (input_monitor.fd_len > 0)
                keyboard_backlight_timeout_tick(ec, cfg, runtime, monotonic_ms(),
                                                input_monitor.last_activity_ms);
            ...
            if (FD_ISSET(sock_fd, &readfds)) {
                ...
                if (client >= 0) {
                    handle_client(client, ec, cfg, states, auto_mode, preset,
                                  preset_len, coolboost_enabled, runtime);
                    close(client);
                    write_runtime_status(ec, cfg, states, *auto_mode, preset,
                                         *coolboost_enabled, names, runtime);
                }
