 ...
 * values.
 */
static void format_temperature_source(char* buf, const size_t buf_len, const char* kind, const char* group)
{
...
const char* refresh_fan_observations(struct ec_device* ec, const struct ans_config* cfg, fan_state states[ANS_MAX_FANS])
{
    ...
    for (int i = 0; i < cfg->fan_len; i++)
    {
        ...
        const int ec_sensor_temp = read_ec_temperature_c(ec, fan->temperature_register);

        const int sensor_temp = ec_sensor_temp >= 0 ? ec_sensor_temp : sensor_read_group_max_c(fan->sensor_group);

        const bool uses_control_group = fan->control_sensor_group[0] && strcmp(fan->control_sensor_group, fan->sensor_group) != 0;
        const int ec_control_sensor_temp = read_ec_temperature_c(ec, fan->control_temperature_register);
        ...
        if (sensor_temp >= 0)
        {
            ...
            if (ec_sensor_temp >= 0)
            {
                snprintf(source, sizeof(source), "component:ec:0x%02x", fan->temperature_register);
            }
            else
            {
                format_temperature_source(source, sizeof(source), "component", fan->sensor_group);
            }

            const int filtered_temp = fan_filtered_sensor_temp(
                fan->id,
                source,
                &states[i].temp_c,
                &states[i].pending_spike_temp_c,
                sensor_temp,
                states[i].temp_seeded,
        ...
        if (control_sensor_temp >= 0)
        {
            ...
            {
                snprintf(source, sizeof(source), "control:ec:0x%02x", fan->control_temperature_register);
            }
            else
            {
                format_temperature_source(
                    source,
                    sizeof(source),
                    "control",
                    uses_control_group ? fan->control_sensor_group : fan->sensor_group
                );
            }

            const int filtered_control_temp = fan_filtered_sensor_temp(
                fan->id,
                source,
        ...
        if (states[i].control_temp_available && states[i].control_temp_c >= cfg->critical_temperature_c)
        {
            if (states[i].critical_temp_samples < cfg->safety.critical_consecutive_samples)
                states[i].critical_temp_samples++;
        }
        else if (states[i].control_temp_available && states[i].control_temp_c <= cfg->safety.critical_release_temperature_c)
        {
        ...
        else if (states[i].critical_temp_samples >= cfg->safety.critical_consecutive_samples && forced_reason[0] == '\0')
        {
